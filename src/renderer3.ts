...
// @ts-ignore
console.log(window.textures.getBlob())

// @ts-ignore
(window as any).textures.onSharedTexture((_event, textureInfo, dupInfo) => {
    ...
    console.debug(textureInfo);
    try {
        const frame = (window as any).textures.getVideoFrame(textureInfo) as VideoFrame;
        if (frame) {
